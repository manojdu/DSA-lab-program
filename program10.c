...
{
int key;
key = num % 100; return key;
}
int getemp(EMP emp[],int key) {
printf("\nEnter emp id: "); scanf("%d",&emp[key].id); printf("\nEnter emp name: "); scanf("%s",emp[key].name); return key;
}
void display()
{
int i, ch;
...
int flag, i, count = 0; flag = 0; if(a[key] == -1)
{
...
for(i=key; i<MAX; i++) if(a[i] == -1)
{
a[i] = getemp(emp, i); flag = 1;
break; }
i = 0;
